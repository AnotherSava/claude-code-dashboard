...
How that looks depends on what you typed. Accept the name Windows Terminal offers you, which is whatever the tab is showing at that moment, and the tab freezes on that glyph: it can read "done" for hours while the agent is busy, and nothing on screen admits it. This is the dangerous one, because the tab still looks like a status, so the widget watches for it and marks the row with a small **≠**, telling you on Telegram too if you have it set up (see [Settings](settings#notifications)). Type your own name instead and the tab simply shows that name, with no status on it at all. The widget cannot flag that one: your text tells it nothing about which session the tab belongs to, and guessing would mean pinning stray windows onto whatever row looked closest. It is the more visible failure anyway, since a tab reading "build" is plainly not reading a status.
