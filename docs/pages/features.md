...
Click the text below a session's name to open a History window for that session — a chronological view of every user prompt, assistant reply, and a separator marking the start of a new session. Useful for scrolling back through a long-running conversation without leaving the dashboard.
