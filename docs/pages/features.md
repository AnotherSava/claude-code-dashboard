...
One thing can stop a tab keeping up: if you rename a tab yourself in Windows Terminal, it holds that name for good and quietly ignores everything the widget writes to it afterwards. The tab then sits on whichever glyph it was wearing at the time, so it can read "done" while the agent is busy. Nothing outside Windows Terminal can undo that — right-click the tab and choose **Reset tab title** and it starts following again. The widget notices when a tab has stopped keeping up and marks the row with a small **≠**, and tells you on Telegram too if you have it set up — see [Settings](settings#notifications).
