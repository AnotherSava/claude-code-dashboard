...
- `stale_tab_alert_ms` — send a message when a session's terminal tab has been showing a stale status for this long. A tab you rename yourself in Windows Terminal keeps that name for good and ignores what the widget writes to it, so it can sit on "done" while the agent is working — right-click the tab and choose **Reset tab title** and it follows again, which is something only you can do. This covers the case where the tab keeps showing a *status* it has stopped updating, which is the one that misleads; a tab renamed to your own text shows no status at all and cannot be told apart from any other window, so it is not flagged. The row is marked with a **≠** the moment the widget notices, so this delay is your chance to spot it and fix the tab before your phone buzzes; the clock runs from when the tab was first caught. Like the context alert, the message is deleted once the tab catches up, the session ends, or you turn this off. `null` or `0` turns it off. The default depends on where your copy came from: a build you deployed yourself starts at `600000` (10 minutes), while the downloadable releases start with it off — it reports a fault in your terminal that the widget can't fix, which is useful once you know the feature and just a puzzling buzz on a fresh install. Set the field either way and your value wins.
