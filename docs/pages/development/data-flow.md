...
**Outbound:** every `emit_sessions_updated` pokes the `SyncDirty` notify; the pusher task debounces 300ms (a 30s heartbeat fires regardless) and POSTs to each `config.sync.peers` entry: a full metadata snapshot of the **local** sessions plus a bounded chunk of dialog backlog — the *oldest* ~256KB of entries above that peer's watermark, in timestamp order — plus, on the first chunk only, the local usage-limit samples newer than that peer's usage watermark (`SyncPush.usage_delta`). Each acknowledged chunk advances the watermark and the next chunk follows immediately, so a peer that was offline drains the whole backlog within one cycle — but only after proving reachable, one bounded POST at a time; a *down* peer costs each failed cycle one bounded build + connect attempt instead of an ever-growing full-backlog serialization. The watermark advances only on a 2xx, so a failed push (peer offline) re-sends the missed entries next time. Both watermark maps are per-run **in-memory** state, never persisted: a restart re-advertises from zero, which the receiver accepts as complete state (`delta_from == 0` for dialogs, the `ts > held_max` dedup for usage). That replay is the only repair path for a wiped `remote_usage/`, since usage has no catch-up endpoint the way dialog does — and persisting a sender-side guess about what the receiver holds would make divergence permanent instead of self-healing. Received remote sessions are never re-broadcast — and remote-driven changes deliberately skip the poke (see inbound below): content can't echo, but the poke itself would, ping-ponging pushes between two devices at the debounce period.
...
Incoming `usage_delta` records are merged per device into `RemoteUsageStore` (`remote_usage/<device>.json` in the app data dir), which `commands::merged_usage_records` unions with the local history when building the Work-intensity chart — the 5h/7d counter is account-wide, so a peer's polls fill the windows this device was closed. A non-empty merge also re-emits `usage_limits_updated`, so an open intensity window re-fetches the merged chart live.
