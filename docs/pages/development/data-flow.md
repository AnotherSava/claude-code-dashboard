...
2. `claude_hook.py` reads the payload, extracts `hook_event_name`, and POSTs `{client: "claude", event: <name>, payload: <verbatim>, console_pids: [...], agent_pid: <pid>}` to `$TAURI_DASHBOARD_URL/api/event` (default `http://127.0.0.1:9077/api/event`). The hook does no classification or config reading — `console_pids` is pure environment gathering (the processes attached to its console plus its ancestor pid chain on Windows, or the ancestor chain alone on macOS, used later for terminal tab titles), and `agent_pid` is the owning Claude process it picks out of that ancestor chain by image name (the nearest `claude` / `claude.exe`), used later to detect a session that exited without a `SessionEnd`.
