...
function Wait-MenuClosed {
    for ($i = 0; $i -lt 20; $i++) {
        $left = @(Find-ProcessWindows -ProcessId $appPid -Class '#32768' -VisibleOnly)
...
$tray = @(Find-ProcessWindows -ProcessId $appPid -Class 'tray_icon_app')
if ($tray.Count -ne 1) { throw "Expected one tray_icon_app window in the dashboard, found $($tray.Count)." }
if (@(Find-ProcessWindows -ProcessId $appPid -Class '#32768' -VisibleOnly).Count -gt 0) { throw 'A dashboard menu is already open. Close it and re-run.' }

$area = Get-PrimaryWorkArea
Invoke-WithPointerAt -X ($area.Right - 40) -Y ($area.Bottom - 8) -Do {
    try {
        [void][WinCapture]::PostMessageW($tray[0], $WM_USER_TRAYICON, [IntPtr]1, [IntPtr]$WM_RBUTTONUP)
        $menu = @()
        for ($i = 0; $i -lt 30 -and $menu.Count -eq 0; $i++) {
            Start-Sleep -Milliseconds 100
            $menu = @(Find-ProcessWindows -ProcessId $appPid -Class '#32768' -VisibleOnly)
        }
        ...
        [void][WinCapture]::PostMessageW($tray[0], $WM_CANCELMODE, [IntPtr]::Zero, [IntPtr]::Zero)
