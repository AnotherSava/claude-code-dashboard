...
export const mockSessions: AgentSession[] = [
  ...
  },
  {
    ...
    previous_prompts: [
      { prompt: 'wire up tray autostart toggle', started_at: now - 6 * min },
      { prompt: 'refactor label policy into its own module', started_at: now - 14 * min },
      { prompt: 'add ipc log channel from webview', started_at: now - 22 * min },
      { prompt: 'initial scaffold of tauri shell', started_at: now - 35 * min },
      { prompt: 'sketch the layout', started_at: now - 48 * min },
    ],
    source: 'claude-code',
  ...
  },
  {
    ...
    previous_prompts: [
      { prompt: 'fix flaky login redirect test', started_at: now - 18 * min },
    ],
    source: 'claude-code',
