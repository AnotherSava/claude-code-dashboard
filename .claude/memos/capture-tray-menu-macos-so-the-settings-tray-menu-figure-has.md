...
The macOS menu is not the Windows one: it carries two macOS-only items the Windows build never shows, the 'Keep awake while working' checkbox and 'Keep awake with lid closed' with its radio group and countdown label, so the figure's alt text ("The tray menu, open: Show / Hide, the checkable toggles, the submenus, ...") should be checked against the macOS frame when it lands.
