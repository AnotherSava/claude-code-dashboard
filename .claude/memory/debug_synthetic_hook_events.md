...
**Alternative — POST the wire envelope directly** (no python / env-var setup, exact control over the JSON): `curl http://127.0.0.1:9077/api/event -d '{"client":"claude","event":"UserPromptSubmit","payload":{"cwd":"/tmp/zzz-test","session_id":"t1","prompt":"do a thing","transcript_path":"/tmp/t.jsonl"}}'`. Gotchas: `client` must be **`"claude"`** (the value `claude_hook.py` sends) — `"claude-code"` is silently `Ignore`d; the raw lifecycle fields nest under `payload` (the hook's envelope shape); and the CSRF guard refuses any `Origin` header except the literal `null` — curl sends none by default, so omit it rather than adding a plausible-looking one (a browser origin → 403). After an app restart (e.g. deploy), rows only re-materialize on the next incoming event, so re-send the flow. To simulate `/clear`: `UserPromptSubmit` → `Stop` → `SessionEnd` (marks a history separator + removes the row) → `SessionStart` (re-derives the same cwd chat_id and restores the dialog).
...
**Watcher-driven corrections need a realpath'd transcript dir (macOS).** When the e2e exercises the transcript *watcher* (append a JSONL line → `notify` → a state correction like `demote_scanned_blocked_to_done`), the transcript file must live at a canonical path. `log_watcher` watches the parent dir and filters FSEvents by **exact** path, but macOS `/tmp` is a symlink to `/private/tmp` and FSEvents reports the canonical `/private/tmp/...` — so a `/tmp/...` transcript path never matches and the watcher **silently never fires** (the hook → state path still works, masking it). Use `os.path.realpath("/tmp")` (or any non-symlinked dir) for both `cwd` and `transcript_path`. Confirmed 2026-06-12 verifying the `Stop`-stale-read `Blocked → Done` correction.
...
**Forcing a duplicate-name collision without touching `custom_names.json`.** To exercise anything keyed on two rows sharing a `display_label` (the `name_shared_by` marker, `attention::resolve_row`'s ambiguity refusal), send a synthetic event whose cwd *basename equals an existing row's custom display name* — e.g. `cwd: "C:/tmp/ccdash-probe/ai-dashboard"` while the real `tauri-dashboard` row is renamed `ai-dashboard`. That yields two rows with one label and needs no edit to the user's names store, so the only cleanup is the usual `SessionEnd`. Used 2026-09-02.
