...
**Plain `brew uninstall --cask` was exercised on 2026-08-22 and is clean** — it quits the app, removes the bundle (backing it up to the Caskroom first), and leaves `SleepDisabled` alone. One side effect worth knowing: the `uninstall launchctl:` stanza *deletes* `~/Library/LaunchAgents/Claude Code Dashboard.plist` rather than just unloading it, and `lib.rs` only calls `autolaunch().enable()` when `is_first_run`. So an install/uninstall/redeploy cycle leaves autostart off until it is re-enabled from the tray. That is correct behavior on both sides — the cask must not leave a LaunchAgent pointing at a deleted binary, and the app must not fight a user who opted out — so don't "fix" either; just re-enable from the tray after testing.
