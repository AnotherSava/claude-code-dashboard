...
Tried and reverted (2026-06-08): extending `idle_probe` to **promote** a non-Working row to Working when the terminal shows `esc to interrupt` (to flip a skill launch to WORK fast). It caused **stuck-in-WORK** — the marker can be present while a row should be `Awaiting`/`Done`, and a one-directional promote strands the row in `Working` that the conservative 2-read demote can't recover.
