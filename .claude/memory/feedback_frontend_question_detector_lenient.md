...
- Backend `is_a_question` → sets `Status::Awaiting` → can trigger Telegram notifications + UI pulse. False-positives are noisy and annoying.
...
So the frontend should err on the side of detecting a question; the backend should err on the side of not. A real case that motivated the divergence: assistant proposes "Want me to delete X? Proposed: ...\n\n...\n\n— confirm and I'll run it." The user replies "y". Backend correctly doesn't fire Awaiting (could be conversational). Frontend should *not* highlight "y" as a new task boundary, so it scans the whole text and catches the earlier "Want me to ... ?" pattern.
